...
interface PersonTalentsProps {
  person: Person;
  onBack: () => void;
}

export function PersonTalents({ person, onBack }: PersonTalentsProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-full overflow-hidden">
      <div className="bg-white px-6 py-6 text-gray-900 border-b border-gray-100 flex items-center shadow-sm relative z-10 transition-colors duration-300">
        <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-gray-100 transition-colors focus:outline-none text-gray-700">
          <Icons.ArrowLeft className="w-6 h-6" />
        </button>
        <h2 className="text-xl font-bold ml-4">Talents de {displayName}</h2>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center text-center">
        <svg className="w-24 h-24 mb-6 text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="currentColor">
        ...
        <p className="text-gray-500 text-lg font-medium">Page Talents à venir</p>
        <p className="text-gray-400 text-sm mt-2">Cette page permettra de visualiser les talents.</p>
      </div>
    </div>
  );
}
